...
\begin{document}
	...
	\begin{exercise}
		Write a function named \code{max'} that returns the maximum element of a list.
	\end{exercise}

	\begin{exercise}
		Write a function named \code{sum'} that computes the sum of the elements of a list of floats.
	\end{exercise}
	...
	\end{marginfigure}
	Having recalled recursion basics, we now proceed to have a bit of higher order fun with a classical sorting algorithm you have encountered last year in your Data Structures module. With that in mind, the next few exercises will help us implement this algorithm from scratch.
	%
	\begin{exercise}\label{ex:higher order function}
		Write a function named \code{functionApplicator} that, given a function, let \code{fn}, and a number, \code{x}, as input, applies \code{fn} to \code{x} to compute and return \code{fn x}.
	\end{exercise}
	%
	A simple but key operation during merge sort is list splitting at two (almost) equal halves.
	\begin{exercise}
	...
	\begin{exercise}\label{ex:merge sort}
		Implement merge sort as a Haskell function \code{mergeSort}. You can (and, maybe, should), utilise \code{merge} from Exercise~\ref{ex:merge} and a higher order function, as the one you developed in Exercise~\ref{ex:higher order function}. Consider how splitting and then merging will be implemented using recursion.
	\end{exercise}
	%
	%
	%
	...
	\begin{marginfigure}
		...
		\end{tikzpicture}
		\caption{A heap of pancakes, awaiting for its sorting.}
